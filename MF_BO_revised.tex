...
\cite{Roustant2020} used a so-called \textit{compound symmetry} covariance function for $\kappa(t_i,t_i'|{\boldsymbol \phi}_{t,i})$ in \cref{eqn32-2}, which assumes a common correlation for all categories.
...
\Cref{Fig-9} shows different schemes of the surrogate-based RBO, which was extensively discussed by~\cite{Moustapha2019}.  
...
%\cite{Yoo2021} introduced an MFO algorithm based on the model-and-optimize approach, solving problem~(\ref{eqn67}) by combining the nonlinear auto-regressive model for two model fidelities and the Monte-Carlo integration.
